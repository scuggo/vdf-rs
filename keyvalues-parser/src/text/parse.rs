...
pub fn parse_(s: &str, escape_chars: bool) -> Result<PartialVdf> {
    ...
    if chars.peek().is_some() {
        Err(Error::TodoMsg("Unexpected trailing bytes after VDF parse".to_string()))
    } else {
...
fn parse_maybe_macro<'text>(
    ...
    if !eat_whitespace(chars) {
        return Err(Error::TodoMsg("Expected whitespace after #base macro".to_string()));
    }
    ...
    if eat_newlines(chars) {
        ...
        Err(Error::TodoMsg("Expected newline after macro definition".to_string()))
    }
}
...
fn parse_quoted_raw_string<'text>(chars: &mut CharIter<'text>) -> Result<&'text str> {
    ...
    while chars.next().ok_or(Error::TodoMsg("Unexpected end of quoted string".to_string()))? != '"' {}
...
fn parse_unquoted_string<'text>(chars: &mut CharIter<'text>) -> Result<&'text str> {
    ...
    match chars.next().ok_or(Error::TodoMsg("Unexpected end of unquoted string".to_string()))? {
        '"' | '{' | '}' | ' ' | '\t' | '\r' | '\n' => return Err(Error::TodoMsg("Invalid character at start of unquoted string".to_string())),
        _ => {}
    }
...
fn parse_quoted_string<'text>(
    ...
    let start_idx = chars.index();
    loop {
        let peeked = chars.peek().ok_or(Error::TodoMsg("Unexpected end of quoted string".to_string()))?;
    ...
    // be `Cow::Borrowed`
    let key = if chars.peek().ok_or(Error::TodoMsg("Unexpected end of quoted string after escape".to_string()))? == '"' {
        assert!(chars.ensure_next('"'));
        ...
        loop {
            let ch = chars.next().ok_or(Error::TodoMsg("Unexpected end of escape sequence in quoted string".to_string()))?;
            match ch {
                '"' => break Cow::Owned(escaped),
                '\\' => match chars.next().ok_or(Error::TodoMsg("Unexpected end of escape sequence in quoted string".to_string()))? {
                    'n' => escaped.push('\n'),
                    'r' => escaped.push('\r'),
                    't' => escaped.push('\t'),
                    '\\' => escaped.push('\\'),
                    '\"' => escaped.push('\"'),
                    '\'' => escaped.push('\''),
                    _ => return Err(Error::TodoMsg("Unknown escape character in quoted string".to_string())),
                },
...
fn parse_obj<'text>(chars: &mut CharIter<'text>, escape_chars: bool) -> Result<Obj<'text>> {
    ...
    while chars.peek().ok_or(Error::TodoMsg("Unexpected end of object, missing closing '}'".to_string()))? != '}' {
        ...
        loop {
            match chars.peek() {
                Some('\r') => {
                    ...
                    match chars.next() {
                        Some('\n') => break,
                        _ => return Err(Error::TodoMsg("Expected newline after carriage return in comment".to_string())),
                    }
                }
                None | Some('\n') => break,
                // Various control characters
                Some('\u{00}'..='\u{08}' | '\u{0A}'..='\u{1F}' | '\u{7F}') => {
                    return Err(Error::TodoMsg("Invalid control character in comment".to_string()))
                }
